...
현대 디스크는 아래와 같은 특징이 있어서, 스케줄링을 디스크가 어느정도는 자체적으로 수행함.

- Disk has multiple outstanding requests
  - SATA NCQ(Native Command Queueing)은 최대 32 requests를 지원
  - SCSI에 원래 있는 기능이고 PATA에는 없었다가, SATA에 추가됨
- Disk schedules requests using its knowledge of head position and track layout
  - SPTF: Shortest Positioning Time First, rotation도 고려해 가장 빠르게 도착하는것부터 함.

### 요약
- HDD is a block device
- Modern HDD interface 는 Logical BLock Addressing 기반. SATA, SAS(Serial Attached SCSI)
  - SATA3는 600MBps지원, SAS는 그거 이상
  - 하드디스크가 읽는 속도가 150~190MB/s밖에 안되는데 뭐하러 SATA3랑 SAS 속도를 저렇게 올려요? 하드디스크의 latency는 어쩔 수 없지만 하드디스크의 bandwidth는 늘릴 수 있다. 하드디스크 여러개 붙이면 된다! 그래서 케이블 속도 중요.
- "Unwritten contract" of HDDs
  - 선형액세스가 random access보다 훨씬 좋음
  - 멀리 떨어진 LBA는 시크타임이 김
  - Data written is equal to data issued (no write amplification)
    - 호스트가 쓰기요청하는 데이터와 실제로 미디어에 쓰는 양이 같다. (SSD는 다르다, HDD도 에러 리커버리때문에 조금 더 클 수 있음)
  - Media does not wear down
  - Storage devices are passive with little background activity
    - 호스트가 뭘 시켜야만 동작하는 수동적인 장치.

&nbsp;

## 14. Solid State Drives
Flash Memory 등장 전까진, 스토리지가 무조건 Magnetic 아니면 Optical이었음. Flash Memory덕분에 전기적으로 기록 가능. 메카니컬한 장치가 필요 없음, 아주 작게 만들 수 있음.

단점이 있다면 파워 없이 오래 지나면 데이터 날아감. 10년은 간다고 하는데 과연 10년이나 갈지..

돌에 새긴건 몇천년이 가는데, 마그네틱이나 옵티컬에 기록한건 오래 못감. 아이러니.

### Flash Memory
DRAM은 density가 높다. 지금은 Flash memory가 더 dense한데, 옛날엔 DRAM이 더 높았음. 가격도 SRAM보다 싸다. 그러나 리프레시 필요하여 전력소모 있음. Volatile함

ROM, Read-only임.

EPROM, 고칠수는 있으나 자외선에 노출되면 데이터 날아감, 뚜껑 덮어야함.

EEPROM, 자외선 대신 전기로 데이터를 지우도록 했음. 칩을 뺴서 기계에 넣으면 지워짐.

EEPROM이 발전해서 Flash memory가 됨. 칩을 뺴지 않아도 지울 수 있어요.

### Flash Memory Cell
원리는 잘 몰라도 됨. 트랜지스터로 만듦. Floating Gate라는 절연된 방에 전자를 가둬서 구현함. 안에 들은 전자를 쫓아내는데에 높은 전압이 필요함. 안에 전자가 들어가면 오래 유지된다. (Non-volatile)

전자가 비면 1, 전자가 들어오면 0. 디폴트는 1.

### Flash Memory Characteristics
1.  Erase-before-write
    - 1->0은 쉽게 됨. 근데 0->1은 비트단위로 하나씩 하는게 불가능함. 내용을 고치려면 Bulk erase 한 뒤 다시 써야함.
2.  Bulk erase
    - 지울때 여러 셀을 한번에 Bulk erase 해야함.

쓰는걸 write, program이라고 부른다.

- 쓰기의 단위, Program Unit:
  - NOR: byte, word 라고 부름
  - NAND: flash page 라고 부름. 처음엔 512B였는데 요즘은 8KiB, 16KiB임. 집적도 늘릴라다보니 이렇게됨.
- 지우기의 단위, Erase Unit:
  - Flash Block. 처음에는 16페이지정도였는데, 지금은 수백페이지임.

Flash Block이 512페이지인데, 한페이지가 8KiB다? 한번 지울때마다 4MiB를 한번에 지워야한다는 뜻.

NOR Flash: code memory. ROM을 대체하는 용도로 쓰였음. BIOS Update할떄 업데이트하는게 NOR Flash임.

NAND Flash: data memory. HDD를 대체.

삼성과 SK 하이닉스가 전세계 NAND Flash 시장의 50~60%를 먹고있는데, 정말 대단한거임. Flash Memory 원천특허는 Toshiba 메모리, 지금의 기옥시아임.

### Logical View of NAND Flash
- 1 Nand Flash = 여러 Flash Block 
- 1 Flash Blcok = 여러 Pages

Block 1개 크기, Page 1개 크기는 기술발전에 따라 점점 커짐

페이지 하나에 Bit Error가 나더라도 고쳐주는 Error Correction Code 저장영역으로 spare area가 존재함. 이거 안쓰면 에러 존나남.

### NAND Flash Types
- SLC NAND: Single Level Cell, 1bit/cell
- MLC NAND: Multi Level Cell (실제로는 2 level임), 2bit/cell
- TLC NAND: 3bit/cell
- QLC NAND: 4bit/cell
- 3D NAND, Vertical NAND

SLC는 방 안에 전자가 있냐 없냐로 구분함. MLC는 전자가 있냐, 1/3있냐, 2/3있냐, 꽉찼냐로 구분함. SLC에서 QLC로 갈수록 용량이 늘겠지만, 에러가 많아지고, 수명이 짧아짐.

MLC, TLC 나올때 사람들이 "이거 쓸 수 있겠냐?" 했는데 문제없이 잘 쓰고있음. 현재 시중의 대부분은 TLC임.

Vertical NAND, 3차원 다이임. 집적도를 엄청 높였고, 더 reliable 함. 처음에는 32단, 지금은 128 넘는단으로 쌓고있음. 몇년동안 다른회사가 못 쫓아왔음.

### NAND 사용처
- UFDs: Universal Flash Drives
- USB 메모리 다 낸드플래시임, SD카드도 다 낸드플래시임
- 스마트폰
  - 옛날에는 Embedded MMC = eMMC 를 썼는데
  - 이젠 다 Universal Flash Storage, UFS로 바뀜
- SSD
- 그 외 온갖 임베디드 장치

### Anatomy of an SSD
메커니컬한건 없고 NAND Flash만 많음. 근데 SSD Controller랑 DRAM도 붙어있음.

컨트롤러는 아래처럼 생김

- Host Interface Controller
- DRAM, DRAM Controller
- 멀티코어 Embedded CPU (보통 세개이상), SRAM
- Flash Controller

SSD는 버스가 여러개임. 다이를 더 붙여서 버스가 여러개가 되면 IO 속도가 올라감. 하나의 Bus 안에 다이 여러개를 붙인다음 파이프라이닝을 해서 대역폭을 늘리기도 함.
...
옛날에는 SSD 전력소모가 더 작았는데, 요즘은 더 많음. 성능 올리려고 칩 여러개를 동시에 IO하다보니 그럼.

HDD는 기계적인 부품이 들어가기때문에, 용량이 작아도 최저가격이 있었음. SSD는 그렇지 않음. M.2라는 아주 작은 폼팩터에도 맞을 수 있음. 그래서 저용량으로 가면 SSD가 더 싸고, 얇고 가볍게 만들기도 더 좋음

### State-of-the-Art @ 2018
2.5인치, 1TB V-NAND * 32 = 30.72TiB.
...
- No in-place update
  - Sector remapping, FTL 필요
- Bit errors
  - Error Correction Codes (ECCs) 필요
- Bad blocks
